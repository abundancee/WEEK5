...
contract abschool {
    ...
    uint256 private salary = 5 ether;
    uint8 staff_id;
    uint8 student_id;
    address owner;
    IERC20 public tokenAddress;

    event StudentRegistered(uint8 indexed studentId, string name, uint8 level, address account);
    event StaffRegistered(uint8 indexed staffId, string name, address account);
